...
void solve_ghost(int start_idx, int end_idx, int cols, int step,double *E_tmp, double *R_tmp, double *E_prev_tmp, double *E ,double *R, double *E_prev, double dt, double alpha){
    ...
    register double e_pre, r_pre, e_prer, e_prel, e_preu, e_pred;
    #ifdef AVX_VEC
        #pragma ivdep
    #endif
    #pragma prefetch
    for(j = start_idx; j <= end_idx; j+=cols) 
    {
        ...
        for(i = 0; i < step; i++) 
        {
            ...
            e_prer=E_prev_tmp[i+1], e_prel=E_prev_tmp[i-1];
            e_preu=E_prev_tmp[i-cols], e_pred=E_prev_tmp[i+cols];
            E_tmp[i] = e_pre+alpha*(e_prer+e_prel-4*e_pre+e_pred+e_preu);
...
void solve_MPI(double **_E, double **_E_prev, double *R, double alpha, double dt, Plotter *plotter, double &L2, double &Linf)
{
 ...
 // Simulated time is different from the integer timestep number
    ...
    int rx, ry, rows, cols, index, i, j, k;
        ...
        {   
            ...
            for(j =1; j <= (rows-4)-(rows-4)%BLOCK_SIZE; j+=BLOCK_SIZE) 
            {
                ...
                for(k=0; k<BLOCK_SIZE; k++)
                {
                    ...
                    E_prev_tmp = E_prev + innerBlockRowStartIndex+1 + (j+k)*cols;
                    #ifdef AVX_VEC
                        #pragma ivdep
                    #endif
                    #pragma prefetch
            ...
            */
            ...
            for(j =1; j <= (rows-4)-(rows-4)%BLOCK_SIZE; j+=BLOCK_SIZE) 
            {
                ...
                {
                    E_tmp = E + innerBlockRowStartIndex+1+ (j+k)*cols;
                    R_tmp = R + innerBlockRowStartIndex+1+ (j+k)*cols;
                    E_prev_tmp = E_prev + innerBlockRowStartIndex+1+ (j+k)*cols;
                    #ifdef AVX_VEC
                        #pragma ivdep
                    #endif
